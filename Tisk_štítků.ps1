...
Write-Host "@init INFO: PowerShell spuštěn, nyní se načte a spustí požadovaný skript."  # Začátek je na samém konci (poslední řádek).
...
$f_log="./x-printer.log"    # Umístění logu
...
$f_xlokace="./x-lokace.csv"                         # NÁHRADNÍ ŘEŠENÍ: CSV soubor obsahující zkratky a popisky lokací (fronta-rezervaci), oddělovačem je ";"
...
$f_xcenik="./x-cenik.csv"                           # CSV soubor ceníku (Formát: audit;DMC_text;nazev;cena;komentar).
...
ForEach ( $ce_polozka IN $cen_load ) {               #trycatchovat
    ...
    $DMC_text = $ce_polozka.DMC_text                 # Interaktivita: 0=přidá se na účtenku bez dalších dotazů, 1=vyhrazeno pro čtenářské průkazky, 2=doptá se na množství/cenu, 3=umožňuje specifikovat název i cenu.
...
# porovnání dat: https://stackoverflow.com/questions/5097125/powershell-comparing-dates --pro vyřazení již vytisknutých rezervací
    ...
    elseif ( $hdata.count -ne 0 -AND $r_error -ne 1 ) {
        ...
        if ( $r_error -ne 1) { Get-Report -report "lokace" }
    ...
    if ( $r_error -ne 1 -AND $no_do -ne 1 ) {
        ...
        ForEach ($rezervace IN $hdata) {
            ...
            if ( $pom.permanent_location -eq ""  ) { $pom.permanent_location = "Asi výměnný fond | 80" }
                   ...
                   CATCH { $Script:Message2Menu+="@func fronta-rezervaci ERROR: Chybějící lokace/data pro jednotku "; [bool]$warn_me=1 }
                 }
...
FUNCTION Get-Report ($report) { #určeno pro nezabezpečené reporty (knihy, lokace)
    SWITCH ($report) {
        "biblio"    {   TRY { $script:tab_xbiblio = Invoke-RestMethod -Method GET -URI "$pub_report_uri`?name=x-biblio_report&annotated=1" } 
        ...
        "lokace"    {   TRY { $script:tab_xlokace = Invoke-RestMethod -Method GET -URI "$pub_report_uri`?name=x-lokace-NEPLATNY_NAZEV&annotated=1" }
                        CATCH { Write-Host "@func Get-Report INFO: Spojení se serverem se nezdařilo. Dojde k pokusu o zpracování lokálního souboru..."
                                $Script:Message2Menu+="@func Get-Report INFO: Nezdařilo se stažení informací o lokacích ze systému KOHA.`n"
                                if ( Test-Path $f_xlokace ) { $script:tab_xlokace = Import-Csv -Path $f_xlokace -Delimiter ";" -Header "Zkratka", "Popisek"
                                                              $Script:Message2Menu+="@func Get-Report INFO: Byl použit záložní soubor $f_xlokace.`n"
                                                            }
                                else {  $Script:Message2Menu+="@func Get-Report INFO: Záložní soubor $f_xlokace nebyl nalezen! Kontaktujte technomága.`n"
                                        #$script:ErrorMessage="Nebylo možné získat data o lokacích."        #protože lokace nejsou tak důležité???
                                        #[bool]$script:r_error=1
                                     }
                              }
                    }
...
FUNCTION vytvor-uctenku {
    ...
    WHILE ( $u_complete -ne 1 )
    {
        ...
        if ( $volba -IN 0..8 -OR $volba -eq "e" -OR $volba -eq "p" ) {
            ...
            elseif ( $volba -le 1 ) { # Registrační poplatky
                ...
                if ( $c_error -ne 1 -AND (($volba -eq 1 -AND $c_typ -ne "D") -OR ($volba -eq 0 -AND $c_typ -eq "D")) ) {
                    ...
                    WHILE ( $choose_incomplete -ne 0 ) {
                        ...
                        if ( $volba -eq 0 ) { $obj_uctenka+=[PSCustomObject]@{ audit="$($pom.audit)"; DMC_text="$($pom.DMC_text) ($c_bcode)"; text="$($pom.nazev) ($c_bcode)"; cena=$($pom.cena)} ; $choose_incomplete=0 }
                ...
                elseif ( $c_error -ne 1 ) { $obj_uctenka+=[PSCustomObject]@{ audit="$($pom.audit)"; DMC_text="$($pom.DMC_text) ($c_bcode)"; text="$($pom.nazev) ($c_bcode)"; cena=$($pom.cena)} } #tohle bude chtít doladit (podmínky)
             }
            ...
            elseif ( $volba -le 7 ) { $obj_uctenka+=[PSCustomObject]@{ audit="$($pom.audit)"; DMC_text="$($pom.DMC_text)"; text="$($pom.nazev)"; cena=$($pom.cena)} } #MVS a rešerše
...
# # # # # # # # # # # AUDITOVANÁ SEKCE # # # # # # # # # # # 
    if ( $aud -eq $true ) {
        ...
        $a_aud="^CFA,40 ^FO,80 ^FB600,,,C, ^FDSTRVZENKA PRO KNIHOVNU^FS ^CF0,25 ^FO,130 ^FB600,,,C, ^FDPobočka KJM: Staré Brno, Křížova 24^FS ^FO15,$vyska ^GB570,1,1^FS ^CF0,28"; $vyska -= 20
        ...
        $a_aud="^XA ^CI28 ^LL$delka $a_aud ^CFA,20 ^FO20,$row1 ^FDVystaveno: $vygenerovano, Suma: $suma CZK^FS ^FO20,$row2 ^FDID: $hash^FS ^BXN,7,200 ^FO70,$DMC_poz ^FD$a2DMC^FS ^XZ"
    }

    SWITCH ( $operace ) {
        0   { tisk -tdata "$a $a_aud" }  #přidat výstup do auditlogu $c
